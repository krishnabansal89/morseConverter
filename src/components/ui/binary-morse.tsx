...
const translations = {
  en: {
    binaryToMorsePlaceholder: "Enter binary code (space separated 8-bit sequences, e.g., 01001000 01000101 01001100 01001100 01001111)",
    morseToBinaryPlaceholder: "Enter Morse code (use dots and dashes, space between characters, three spaces between words)",
    onlyBinaryAllowed: "In binary mode, only 0, 1, and spaces are allowed",
    ...
    visual: "Visual",
    swap: "Swap",
    convertedMorseWillAppearHere: "Morse code will appear here",
    binaryWillAppearHere: "Binary will appear here",
    errorFailedToInitAudio: "Failed to initialize AudioContext:",
    headingMorseCode: "Morse Code",
    headingBinary: "Binary",
    backspaceDisabled: "Backspace is disabled in live mode"
  }
}

// International Morse Code Map (same as original)
...
export default function BinaryMorseConverter({
  initialText = "",
  binaryToMorse = true
}: {
  initialText?: string,
  binaryToMorse?: boolean
}) {
  const strings = translations.en;
  const [inputText, setInputText] = useState(initialText || "")
  const [outputText, setOutputText] = useState("")
  const [mode, setMode] = useState<"binary-to-morse" | "morse-to-binary">(binaryToMorse ? "binary-to-morse" : "morse-to-binary")
  ...
  const binaryToText = (binary: string): string => {
    const binaryArray = binary.trim().split(/\s+/).filter(b => b);
    return binaryArray
      .map(bin => {
        // Ensure it's 8 bits
  ...
  useEffect(() => {
    const placeholder = mode === "binary-to-morse"
      ? strings.binaryToMorsePlaceholder
      : strings.morseToBinaryPlaceholder
  ...
  const processNewCharacters = (newText: string) => {
    ...
    let newOutput = ""
    if (mode === "binary-to-morse") {
      // Binary -> Text -> Morse
      const text = binaryToText(newText)
      newOutput = textToMorse(text)
      setOutputText(newOutput)
      
      // Queue morse characters for audio/visual
      if (audioEffectSelected || visualEffectSelected) {
        const previousOutput = mode === "binary-to-morse" ? textToMorse(binaryToText(newText.slice(0, previousLength))) : ""
      ...
      // Morse -> Text -> Binary
      const text = morseToText(newText)
      newOutput = textToBinary(text)
      setOutputText(newOutput)
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (mode === "binary-to-morse") {
      const sanitizedValue = value.replace(/[^01 ]/g, "");
      if (sanitizedValue !== value) {
        showAlert(strings.onlyBinaryAllowed, 'error', 3000);
      }
  ...
  const toggleMode = () => {
    ...
    const newMode = mode === "binary-to-morse" ? "morse-to-binary" : "binary-to-morse"
    setMode(newMode)
  ...
  const handleDownload = () => {
    if (outputText) {
      ...
      element.download = `binary-morse-conversion-${Date.now()}.txt`
  ...
  const handleHelp = () => {
    ...
    if (!inputText && !outputText) {
      helpMessage = "Enter binary/morse to see live conversion";
    } else if (mode === "binary-to-morse") {
      helpMessage = "Type Binary to convert to Morse code";
    } else {
      helpMessage = "Type Morse to convert to Binary";
    }
      ...
      <div className="w-full mx-auto rounded-lg overflow-hidden border border-gray-200">
        <div className="grid md:grid-cols-2 relative">
            ...
            <div className="flex md:justify-between justify-around items-center p-4 border-b border-gray-200 md:w-full md:flex-row flex-col gap-y-2">
              <div className="flex items-center">
                <span className="md:text-lg text-2xl font-bold text-primary">
                  {mode === "binary-to-morse" ? strings.headingBinary : strings.headingMorseCode}
                </span>
              </div>
          ...
          <div>
            <div className="flex md:justify-between justify-around items-center p-4 border-b border-gray-200 text-[#372824] md:w-full md:flex-row flex-col gap-y-2">
              <div className="flex items-center">
                <span className="md:text-lg text-2xl text-primary font-bold">
                  {mode === "binary-to-morse" ? strings.headingMorseCode : strings.headingBinary}
                </span>
              </div>
            ...
            <div className="p-4 text-gray-800 whitespace-pre-wrap min-h-[200px] md:min-h-[300px] text-xl pl-8 dark:text-gray-100 break-all">
              {outputText || (
                <span className="text-gray-400 text-md">
                  {mode === "binary-to-morse"
                    ? strings.convertedMorseWillAppearHere
                    : strings.binaryWillAppearHere
                  }
                </span>
              )}
            </div>
